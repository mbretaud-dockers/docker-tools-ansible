...
# Optional deploy key
if [ ! -z "$DEPLOY_KEY" ] && [ ! -f "/opt/application/ansible/.ssh/id_rsa" ]; then
    mkdir -p /opt/application/ansible/.ssh/
    echo "${DEPLOY_KEY}" > /opt/application/ansible/.ssh/id_rsa
    chmod 0600 /opt/application/ansible/.ssh/id_rsa
fi

# Loadkey into ssh-agent if key exist
if [ -f "/opt/application/ansible/.ssh/id_rsa" ]; then
    eval $(ssh-agent)
    ssh-add /opt/application/ansible/.ssh/id_rsa
fi
